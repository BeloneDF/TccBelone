...
void getData() {
  ...
  if (httpResponseCode > 0) {
    ...
    if (root.containsKey("rele_acionamento") && root.containsKey("rele_operacao")) {
      estado_rele_partida = root["rele_acionamento"];
      estado_rele_operacao = root["rele_operacao"];
      digitalWrite(RELE_OPERACAO_PIN, estado_rele_operacao ? LOW : HIGH);
      digitalWrite(RELE_PARTIDA_PIN, estado_rele_partida ? LOW : HIGH);
      
      if (root.containsKey("manual")) {
        bool manualMode = root["manual"];
        if (!manualMode) {
          // O modo manual está desativado, então execute o código do método post
          float distance = measure_distance();
          aciona_reles(distance);
          post(distance, estado_rele_operacao, estado_rele_partida);
        }
        ...
        Serial.println("Resposta da API não contém informações do modo manual.");
      }
    } else {
      Serial.println("Resposta da API não contém informações dos relés.");
    }
  } else {
    Serial.print("Error in HTTP request: ");
    Serial.println(httpResponseCode);
